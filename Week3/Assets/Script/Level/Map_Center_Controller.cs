...
public class Map_Center_Controller : BaseLevelController
{
    ...
    private void UpdateMainMapDoors()
    {
        DoorControler[] allDoors = FindObjectsByType<DoorControler>(FindObjectsSortMode.None);

        foreach (DoorControler door in allDoors)
        {
            switch (door.doorIdentifier)
            {
                case "4":
                    if (GameDataModel.Instance.IsLevelCleared("4"))
                        door.closeDoor();
                    break;

                case "5":
                    if (GameDataModel.Instance.IsLevelCleared("5"))
                        door.closeDoor();
                    break;

                case "6":
                    if (GameDataModel.Instance.IsLevelCleared("6"))
                        door.closeDoor();
                    break;
            }
        }
